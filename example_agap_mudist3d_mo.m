...
mutfun = @(x,f) x + ranrange(0,0.1,3); % Mutation: small random movement
repfun = @(x,y,fx,fy) (x+y)/2; % Reproduction: average of two individuals
ranfun = @() ranrange(0,2,3); % Random individual
prifun = @(x) fprintf('%f %f %f ',x(1),x(2),x(3)); % Print an individual
